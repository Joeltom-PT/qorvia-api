...
import com.qorvia.paymentservice.client.EventManagementClient;
...
//import com.qorvia.paymentservice.rabbitmq.Publisher;
...
import com.qorvia.paymentservice.repository.PayoutTransactionRepository;
...
//    private final Publisher publisher;
    ...
    @Override
    public ResponseEntity<RefundDTO> refundPayment(RefundRequestDTO refundRequest) throws StripeException {
        try {
            ...
            if (payment == null || payment.getPaymentStatus() != PaymentStatus.COMPLETED) {
                return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(createRefundDTO(refundRequest.getSessionId(), "FAILED", "Invalid session ID or payment is not completed."));
            }
            ...
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body(refundDTO);
        } catch (Exception e) {
