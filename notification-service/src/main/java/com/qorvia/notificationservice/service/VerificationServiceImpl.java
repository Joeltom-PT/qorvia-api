...
public class VerificationServiceImpl implements VerificationService {
    ...
    @Override
    public ResponseEntity<ApiResponse<String>> organizerEmailVerification(String email) {
        if (email == null) {
            throw new IllegalArgumentException("Email must not be null");
        }
        ...
        return ResponseEntity.ok(new ApiResponse<>(200, "Verification email sent successfully", verificationLink));
    }

    @Override
    public ResponseEntity<ApiResponse<OrganizerVerificationResponse>> organizerEmailVerify(String token) {
        if (token == null) {
        ...
        try {
            ...
            return ResponseEntity.badRequest().body(new ApiResponse<>(400, "Invalid token format", null));
        }
        ...
        if (storedEmail != null && storedEmail.equals(providedEmail)) {
            ...
            return ResponseEntity.ok(new ApiResponse<>(200, "Email verification successful", response));
        }

        log.error("Invalid token or email. Token: {}, Stored Email: {}, Provided Email: {}", token, storedEmail, providedEmail);
        return ResponseEntity.badRequest().body(new ApiResponse<>(400, "Invalid token or email", null));
    }
