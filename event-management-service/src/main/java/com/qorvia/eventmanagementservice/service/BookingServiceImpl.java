...
import com.qorvia.eventmanagementservice.dto.client.event.PaymentStatusChangeEvent;
...
public class BookingServiceImpl implements BookingService{
    ...
    public ResponseEntity<?> processBooking(Long userId, String email, EventBookingDTO eventBookingDTO) {
        ...
        if (event.isOnline()) {
            ...
            if (onlineTicket.getPrice() == null && onlineTicket.isFreeEvent()){
                totalAmount = 0;
            } else {
                totalAmount = (onlineTicket.getDiscountType() != null && !onlineTicket.getDiscountType().equals("") && !onlineTicket.getDiscountType().isEmpty() && onlineTicket.getDiscountType().equals("percentage"))
                        ? onlineTicket.getPrice() - ((double) (onlineTicket.getPrice() * onlineTicket.getDiscountValue()) / 100)
                        : onlineTicket.getPrice() - onlineTicket.getDiscountValue();
            }
        ...
        if (finalAmount >= 10){
            ...
            ResponseEntity<PaymentInfoDTO> responseEntity = paymentClient.getPaymentInfo(paymentRequestDTO);
            PaymentInfoDTO paymentInfoDTO = responseEntity.getBody();
    ...
    public ResponseEntity<?> cancelBooking(String bookingId, Long userId) {
        ...
        if (booking.getPaymentStatus() == PaymentStatus.COMPLETED) {
            ...
            try {
                ResponseEntity<?> refundResponse = paymentClient.paymentRefundRequest(refundRequestDTO);
                RefundDTO refundDTO = (RefundDTO) refundResponse.getBody();
